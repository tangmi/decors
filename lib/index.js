...
function getModules(app, appdir, options) {
	...
	if (options.watch && options.inject) {
		console.log('Unfortunately, --watch and --inject can\'t be used at the same time. :\'(');
		console.log('(This will eventually be solved and fixed)');
		process.exit(1);
	}
	...
	if (options.inject) {
		...
		modules.push('./modules/inject.js');
	}
...
function startServer(options, onStart) {
	...
	log.info();
	log.info(' decors' + ('@' + package.version).grey);
	log.info();
	...
	module.exports._server.listen(app.get('port'), function() {
		log.info('Decors server started on port', app.get('port'));
		log.info();
		if (onStart) {
			onStart();
		}
