var log = require('../log'),
	fs = require('fs'),
	path = require('path'),
	url = require('url'),
	mime = require('mime'),
	http = require('http'),
	https = require('https');

module.exports = function(appdir, options) {
	if (options.backend.lastIndexOf('/') === options.backend.length - 1) {
		options.backend = options.backend.substring(0, options.backend.length - 1);
	}
	log.info('Rerouting API calls to:', options.backend);
	return function(req, res, next) {

		//add some middleware to save the raw body
		req.setEncoding('utf8');
		req.rawBody = '';
		req.on('data', function(chunk) {
			req.rawBody += chunk;
		});
		req.on('end', function() {
			reroute();
		});

		//actually reroute the backend call

		function reroute() {
			//for the empty directory index.html edge case
			var indexExists = fs.existsSync(path.join(appdir, req.path, 'index.html'));
			if (fs.existsSync(path.join(appdir, req.path)) && req.path.length > 1) {
				next();
			} else if (req.path.lastIndexOf('/') == req.path.length - 1 && indexExists) {
				next();
			} else {
				if (req.path.indexOf('/') !== 0) {
					req.path = '/' + req.path;
				}

				//could this be done above only once?
				var remotepath = options.backend + req.path;
				if (remotepath.indexOf('http') !== 0) {
					remotepath = 'http://' + remotepath;
				}

				var querystring = url.parse(req.url).query;
				if (typeof querystring !== 'undefined' && querystring != null) {
					remotepath += '?' + querystring;
				}

				var regex = new RegExp('^([a-zA-Z]+)://([^:^/]+)(?:\:([0-9]+))?(/.*)?$');
				var protocol = remotepath.indexOf('https') === 0 ? https : http;

				var reqoptions = {
					host: remotepath.replace(regex, '$2'),
					port: remotepath.replace(regex, '$3') || 80,
					path: remotepath.replace(regex, '$4'),
					method: req.method,
					headers: req.headers
				};

				if (options.port == reqoptions.port) {
					res.send(404);
					log.response(req.method, '404', 'remote', remotepath, ('(recursive call)').grey);
					return;
				}

				var remoterequest = protocol.request(reqoptions, function(data) {
					res.removeHeader('x-powered-by');
					res.statusCode = data.statusCode;
					var gzipped = false; // overrides the useutf8 setting
					for (var header in data.headers) {
						if (data.headers.hasOwnProperty(header)) {
							var value = data.headers[header];
							header = header.replace(/[a-z]+/g, function(match) {
								return match[0].toUpperCase() + match.substring(1);
							});
							res.setHeader(header, value);

							if (header === 'Content-Encoding') {
								if (value.indexOf('gzip') !== -1 || value.indexOf('deflate') !== -1) {
									gzipped = true;
								}
							}
						}
					}

					var output = '';

					var buffer = new Buffer(+data.headers['content-length']);
					var offset = 0;

					var mimetype = mime.lookup(req.path);

					var useutf8 = !gzipped || /text\/.+/.test(mimetype); //determines the data transfer encoding

					if (/image\/.+/.test(mimetype)) {
						useutf8 = false;
					}

					if (useutf8) {
						data.setEncoding('utf8');
					} else {
						data.setEncoding('binary');
					}

					data.on('data', function(chunk) {
						if (useutf8) {
							output += chunk;
						} else {
							try {
								buffer.write(chunk, offset, 'binary');
								offset += chunk.length;
							} catch (e) {
								res.send(404);
							}
						}
					});

					data.on('end', function() {
						if (useutf8) {
							res.write(output);
							res.end();
						} else {
							res.write(buffer.toString('binary', 0, offset), encoding = 'binary');
							res.end();
						}

						log.response(req.method, res.statusCode, 'remote', remotepath);
					});
				});

				remoterequest.write(req.rawBody);

				remoterequest.on('error', function(e) {
					res.contentType('text/plain');
					res.send(404);
					log.response(req.method, '404', 'remote', remotepath, ('(' + e.message + ')').grey);
				});

				remoterequest.end();
			}
		}
	};
};